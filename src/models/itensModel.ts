...
class ItemDAO {
  static async itemInsert(item: Item) {
    const sql =
      "INSERT INTO item (name, description, image, item_type) VALUES ($1, $2, $3, $4)"
  ...
  static async itemUpdate(item: Item, id: number) {
    const sql =
      "UPDATE item SET name = $1, description = $2, image = $3, item_type = $4 WHERE item_id = $5"
  ...
  static async itemList() {
    const sql =
      "SELECT item_id AS id, name, description, image, item_type AS type, created_at, updated_at FROM item"
  ...
  static async getItemById(id: number) {
    const sql =
      "SELECT name, description, image, item_type AS type, created_at, updated_at FROM item WHERE item_id = $1"
