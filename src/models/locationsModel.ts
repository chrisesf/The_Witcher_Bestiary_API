...
class LocationDAO {
  ...
  static async locationUpdate(location: Location, id: number) {
    const sql =
      "UPDATE location SET name = $1, description = $2  WHERE location_id = $3"
  ...
  static async locationList() {
    const sql =
      "SELECT location_id AS id, name, description, created_at, updated_at FROM location"
  ...
  static async getLocationById(id: number) {
    const sql =
      "SELECT name AS nome, description, created_at, updated_at FROM location WHERE location_id = $1"
